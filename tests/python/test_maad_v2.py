...
def test_insert_maad_v2_rows_persists_all_three_tables() -> None:
    ...
    structure = conn.execute(
        'SELECT source_id, bucket_start, ip_version, structure_json_sa, metadata_json_da FROM structure_stats_v2'
    ).fetchone()
    spectrum = conn.execute(
        'SELECT source_id, bucket_start, ip_version, spectrum_json_da FROM spectrum_stats_v2'
    ).fetchone()
    dimensions = conn.execute(
        'SELECT source_id, bucket_start, ip_version, dimensions_json_sa FROM dimension_stats_v2'
    ).fetchone()

    assert structure[:3] == ('oh_ir1_gw', 1744733100, 4)
    assert json.loads(structure[3]) == [{'q': -0.5, 'tauTilde': -0.98, 'sd': 0.01}]
    assert json.loads(structure[4])['totalAddrs'] == 20
    assert json.loads(spectrum[3]) == [{'alpha': 0.75, 'f': 0.60}]
    assert json.loads(dimensions[3]) == [{'q': 1, 'dim': 0.48}]

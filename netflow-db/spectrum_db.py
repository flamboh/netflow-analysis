...
def compute_spectrum(ips: set[str]) -> list[dict]:
    """Compute spectrum for a set of IP addresses.
    
    Calls the Zig spectrum binary via stdin and parses the CSV output.
...
class Result:
    ...
    def write_result(self, bucket_start, bucket_end, conn):
        ...
        points_sa = compute_spectrum(self.ips_sa)
        points_da = compute_spectrum(self.ips_da)
...
def determine_start_day():
    ...
    cursor = conn.cursor()
    row = cursor.execute(
        "SELECT MAX(bucket_end) FROM spectrum_stats WHERE granularity = '5m'"
    ).fetchone()
    conn.close()

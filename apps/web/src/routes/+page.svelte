...
<main class="mx-auto flex max-w-[90vw] flex-col gap-4 px-4 py-8 sm:px-2 lg:px-4">
	{#if loading}
		<section class="rounded-lg border bg-white p-6 text-gray-500 shadow-sm dark:border-dark-border dark:bg-dark-surface dark:text-gray-400 dark:shadow-none">
			Loading datasets...
		</section>
	{:else if error}
		<section class="rounded-lg border border-red-200 bg-red-50 p-6 text-red-700 shadow-sm dark:border-red-900 dark:bg-red-950 dark:text-red-400 dark:shadow-none">
			{error}
		</section>
	...
	{:else}
		<div class="grid gap-4 md:grid-cols-2">
			{#each datasets as dataset (dataset.datasetId)}
				<button
					type="button"
					class="cursor-pointer rounded-lg border bg-white p-5 text-left shadow-sm transition hover:border-blue-400 hover:shadow dark:border-dark-border dark:bg-dark-surface dark:shadow-none dark:hover:border-blue-500 dark:hover:bg-dark-subtle"

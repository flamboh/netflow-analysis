			...
			await ensureCachedWindow<IpStatsBucket>({
				...
				fetchRange: async (range) => {
					const response = await fetch(
						`/api/ip/stats?${new URLSearchParams({
...
<div class="rounded-lg border border-gray-200 bg-white shadow-sm">
	<div
		...
		data-drag-handle
	>
		<h3 class="text-sm font-semibold text-gray-900">Unique IP Counts</h3>
		<DragGrip />
	</div>
	<div class="p-4">
		...
		>
			{#if loading}
				<div class="flex h-full items-center justify-center">
					<p class="text-sm text-gray-400">Loading IP data...</p>
				</div>
			{:else if error}
				<div class="flex h-full items-center justify-center">
			...
			{:else if buckets.length === 0}
				<div class="flex h-full items-center justify-center">
					<p class="text-sm text-gray-400">No IP data for the selected window.</p>
				</div>
			{:else}
				<div class="h-full">

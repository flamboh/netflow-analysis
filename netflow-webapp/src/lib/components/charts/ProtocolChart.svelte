...
<div class="border-border bg-surface rounded-xl border shadow-sm">
	<div
		...
		data-drag-handle
	>
		<h3 class="text-text-primary text-lg font-semibold">Unique Protocol Counts</h3>
		<DragGrip />
	</div>
	<div class="p-4">
		...
		>
			{#if loading}
				<div class="flex h-full items-center justify-center">
					<div class="text-text-muted">Loading protocol data...</div>
				</div>
			{:else if error}
				<div class="flex h-full items-center justify-center">
			...
			{:else if buckets.length === 0}
				<div class="flex h-full items-center justify-center">
					<div class="text-text-muted">No protocol data for the selected window.</div>
				</div>
			{:else}
				<div class="h-full">

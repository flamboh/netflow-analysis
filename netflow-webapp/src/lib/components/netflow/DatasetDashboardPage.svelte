...
<main class="mx-auto flex max-w-[90vw] flex-col gap-2 px-4 py-8 sm:px-2 lg:px-4">
	<h1 class="px-1 text-xl font-semibold text-gray-900">{props.title ?? props.dataset}</h1>
	...
	<div role="list" aria-label="Reorderable charts" class="flex flex-col gap-2">
		{#each chartOrder as chartId (chartId)}
			<section
				role="listitem"
				data-chart-card
				class={`rounded-lg ${dropTargetChartId === chartId && draggedChartId && draggedChartId !== chartId ? 'ring-cisco-blue/40 ring-2 ring-offset-2' : ''}`}
				ondragstart={(event) => {
